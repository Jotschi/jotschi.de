...
I have been using graph databases for a while (mostly Neo4j) and i thought it might be a good idea to write down some of the things i noted while using "Neo4j":http://neo4j.com/ in combination with "Spring Data Neo4j (SDN)":http://projects.spring.io/spring-data-neo4j/. 
...
The database is very easy to embedd and comes with a powerful query language (cypher). I don't know whether there are any other dedicated OGM/ORM layers for Neo4j besides of SDN and blueprint based OGMs.
...
The High Availability mode (Master/Slave Replication) can also be used when embedding the database. I wrote a "dummy project":https://github.com/Jotschi/neo4j-ha-example a while ago that contains a working example.

h2. OrientDB

"OrientDB":http://orientdb.com/ is also open source. There is no cypher but you can use Orient SQL. Embedding is also very easy and the licensing with Apache 2 license is very developer friendly. Tinkerpop support is very good.
...
"Titan DB":http://thinkaurelius.github.io/titan/ is an interessting database. The storage layer for this graph database is interchangeable. You can use "Berkeley DB":http://en.wikipedia.org/wiki/Berkeley_DB which is quiet fast but it basically limits the size of nodes you can store and you can't use clustering.
...
h1. OGM - Object Graph Mapping

I created a "github project":https://github.com/Jotschi/graph-ogm-examples that contains examples for all mentioned OGM's and graph databases.

h2. Spring Data Neo4j
...
[Example project](https://github.com/Jotschi/graph-ogm-examples/tree/master/spring-data-neo4j-3.x)
...
Instead i used the "Neo4jTemplate":http://docs.spring.io/autorepo/docs/spring-data-neo4j/current/api/org/springframework/data/neo4j/support/Neo4jTemplate.html class in order to populate the returned enitity. When no @Fetch was specified only a shallow object with no properties is returned. Using the neo4jTemplate.fetch() this shallow object could be loaded.
...
[Example project](https://github.com/Jotschi/graph-ogm-examples/tree/master/spring-data-neo4j-4.x)
...
"Documentation 4.0.0.M1":http://docs.spring.io/spring-data/neo4j/docs/4.0.0.M1/reference/html/

h2. Tinkerpop

"Tinkerpop":http://tinkerpop.incubator.apache.org/ is a collection of APIs that allow transparent and easy interfacing with graph databases. The blueprint API is the most low level api which is used to wrap the graph databases native API. By doing so it provides a standarized API which other APIs can use to interface with a graph db through this API layer. The API layer is very thin. There are various wrappers for many graph databases. I have used the "blueprint neo4j implementation":https://github.com/tinkerpop/blueprints/tree/master/blueprints-neo4j2-graph.
...
The "Ferma Benchmark":https://github.com/freemo/Ferma-benchmark contains mesurements for Frames, Totorom and Ferma.

h3. Tinkerpop 2 - Frames

"Example project":https://github.com/Jotschi/graph-ogm-examples/tree/master/frames

The Frames API uses annotations similar to SDN and thus switching from SDN to Frames is not that hard. Indices have to be created seperatly. Tinkerpop does not support cypher. You would need to write your "statements in gremlin":http://sql2gremlin.com/ instead. The project seems not very active and Frames will not be part of Tinkerpop 3.
...
h3. Tinkerpop - Totorom

"Example project":https://github.com/Jotschi/graph-ogm-examples/tree/master/totorom
...
* The "documentation":http://wiki.syncleus.com/index.php/Ferma is quite good.
